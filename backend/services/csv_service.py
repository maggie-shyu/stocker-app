...
class CsvService:
    ...
    def get_commission_discount_rate(self) -> float:
        ...
        with open(path, encoding="utf-8") as fh:
            return float(json.load(fh).get("commission_discount_rate", 0.0))
    ...
    def search_stocks(self, query: str, *, limit: int = 20) -> list[StockLookup]:
        ...
        matches = [
            s for s in self.read_stocks()
            if query in s.code.lower() or query in s.name.lower()
        ]
