...
describe("App", () => {
  ...
  it("renders the dashboard shell and KPI cards", async () => {
    ...
    expect(await screen.findByText("0050報酬率")).toBeInTheDocument();
    expect(await screen.findByText("年化報酬率")).toBeInTheDocument();
    expect((await screen.findAllByText("+59.00%")).length).toBeGreaterThan(0);
  ...
  it("renders transactions, holdings, realized, cashflow, and settings routes", async () => {
    ...
    expect(await screen.findByText(/\+\s1,234,567/)).toBeInTheDocument();
  ...
  it("switches the selected admin table and reloads the matching rows", async () => {
    ...
    const select = await screen.findByLabelText("Supabase");
  ...
  it("syncs stock code and name in both directions and keeps fee preview params intact", async () => {
    ...
    expect(await screen.findByText("手續費 20 · 支出 100,020")).toBeInTheDocument();

    await user.click(screen.getByRole("button", { name: "賣" }));
    expect(await screen.findByText("手續費 20 · 稅金 5 · 收入 99,975")).toBeInTheDocument();
