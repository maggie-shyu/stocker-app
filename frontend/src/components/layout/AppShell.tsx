...
const navGroups = [
  {
    ...
    items: [
      ...
      { to: "/realized", label: "平倉盈虧", icon: BarChart3 },
    ],
  },
  {
    label: "交易管理",
    items: [
      ...
      { to: "/cashflow", label: "資金異動", icon: Banknote },
    ],
  },
  {
    label: "系統設定",
      ...
      >
        <div className={`flex h-20 items-center gap-3 px-5 ${isCollapsed ? "lg:justify-center lg:px-3" : ""}`}>
          ...
          <div className={isCollapsed ? "lg:hidden" : ""}>
            ...
            <div className="text-sm font-medium text-muted">股客</div>
          </div>
        </div>
        ...
        <nav className={`flex gap-1 overflow-x-auto px-3 pb-3 pt-4 lg:block lg:space-y-2 lg:overflow-visible ${isCollapsed ? "lg:px-3" : "lg:px-5"}`}>
          {navGroups.map((group) => (
            <section key={group.label} className="flex gap-1 lg:block lg:min-w-0 lg:space-y-1">
              <div className="flex gap-1 lg:block lg:space-y-1">
                {group.items.map((item) => (
                  ...
                  >
                    ...
                    <span className={`hidden sm:inline${isCollapsed ? " lg:hidden" : ""}`}>{item.label}</span>
                  </NavLink>
                ))}
              </div>
            </section>
